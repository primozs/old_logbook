...
proc read2007on(): seq[LogbookEntry] =
  ...
  defer: f.close()
  let data = f.readAll()
  let entries = fromCsv[LogbookEntry2007Csv](data, seq[LogbookEntry2007Csv],
      hasHeader = true)
  for ec in entries:
...
proc mergeOldCsvFiles*() =
  ...
  df.writeCsv("output" / "logbook-sorted.csv")
  df.showBrowser("output" / "logbook.html")
